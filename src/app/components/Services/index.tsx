...
function Box() {
  return (
    <motion.div variants={variants} className="box border-2 border-yellow-700 p-5 flex justify-between flex-col items-start rounded-md">
      ...
      <button className="btn w-32 hover:text-yellow-600">Go</button>
    </motion.div>
  );
}
export default function Services() {
  ...
  return (
    <motion.div variants={variants} ref={ref}  initial='initial' animate={isInView && 'animate'}  className="services flex flex-col justify-between">
      <div className="text_container flex-[1] self-end flex items-center gap-7 [&>hr]:w-96  [&>hr]:border-t [&>hr]:border-gray-500">
      ...
      <motion.div variants={variants} className="title_container self-center flex-[2] space-y-2">
        <div className="title flex justify-center items-center gap-5">
        ...
        <div className="title flex justify-center items-center gap-5 flex-wrap">
          <h2 className="text-7xl">
            <motion.b whileHover={{color:'orange'}}>For your</motion.b> <span className="font-thin">Business.</span>
          </h2>
          <button className="px-8 py-4 bg-yellow-600 text-black rounded-full border-none text-xl hover:bg-yellow-500">What we do?</button>
        </div>
      </motion.div>
      <motion.div variants={variants} className="list_container flex-[2] flex max-w-[1500px] mx-auto space-x-5">
        <Box/>
        <Box/>
        <Box/>
      </motion.div>
